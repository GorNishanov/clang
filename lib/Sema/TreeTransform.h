...
TreeTransform<Derived>::TransformCoroutineBodyStmt(CoroutineBodyStmt *S) {
  ...
  Stmt *ResultDecl = S->getResultDecl();
  assert(ResultDecl && "the return object is expected to be valid");
  StmtResult ResultDeclTransformed = getDerived().TransformStmt(ResultDecl);
  if (ResultDeclTransformed.isInvalid())
    return StmtError();
  Builder.ResultDecl = ResultDeclTransformed.get();

  Stmt *ReturnStmt = S->getReturnStmt();
  assert(ReturnStmt && "the return statement is expected to be valid");
  StmtResult ReturnStmtTransformed = getDerived().TransformStmt(ReturnStmt);
  if (ReturnStmtTransformed.isInvalid())
    return StmtError();
  Builder.ReturnStmt = ReturnStmtTransformed.get();
